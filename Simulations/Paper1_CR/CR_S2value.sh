for l in 1 #{1..2} # cause 1 probabilty for CR # change to just 1 for simple_exp
  do for k in 1 #{1..2}  #crit(both surv and endpoint = stick together), 1-2
    do for j in 1 #{1..2}  #size = 1,2
      do for i in 1 #{1..2} #propensity = 1,2
        do for h in 1 #{1..2} #{1..4} beta = 1,2,3,4
          #ncauses = 2
            do for g in 1 #{1..2} # censor is either low (1) or high (2)
   	          #endpoint = CR
