        ...
        do for h in {1..2} #{1..4} beta = 1,2,3,4
          #ncauses = 2
            do for g in {1..2} # censor is either low (1) or high (2)
   	          #endpoint = CR
                do sbatch -t 07-05:00:00 -n 3 --mail-type=fail --mail-user=cwzhou@email.unc.edu --mem=200G --error=slurm-%j.out --output=slurm-%j.out CR_S0run.sh CR01.Simulation_Run.R 1 $g 1 $h $i $j $k $k $l $1  #$1 = date
