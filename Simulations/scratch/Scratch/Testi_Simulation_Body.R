...
#### library
library(dtrSurv)
# source("Testi_GenerateDataset.R")
...
if (criterion_phase1 == "mean") {
  val.fn_phase1 <- function(x) {mean(x, na.rm = TRUE)}
} else if (criterion_phase1 %in% c("prob", "prob.mean.combo")) {
  val.fn_phase1 <- function(x) {mean(x >= crit.value_phase1, na.rm = TRUE)}
}
if (criterion_phase2 == "mean") {
  val.fn_phase2 <- function(x) {mean(x, na.rm = TRUE)}
} else if (criterion_phase2 %in% c("prob", "prob.mean.combo")) {
  val.fn_phase2 <- function(x) {mean(x >= crit.value_phase2, na.rm = TRUE)}
}
message("Phase1 Criterion: ", criterion_phase1)
message("Phase2 Criterion: ", criterion_phase2)

filename.tmp <- gsub("\\.rds", "_tmp.rds", filename)
### simulation
result <- data.frame(sim = 1:n.sim, observed = NA, csk = NA, gkLM = NA, gkRF = NA, 
                     dw = NA, zom = NA,
                     time.obs = NA, time.csk = NA, time.gkLM = NA, time.gkRF = NA,
                     time.dw = NA, time.zom = NA, percent.censor = NA, n = NA)  # time for each method (both policy est and eval)
...
if (endpoint == "CR"){
  arg_list =   list(
    n.sample = n, tau = tau, tick = 0.01,  # structural parameters
    censor_rate = censor_rate,
    ncov = ncov, ncauses = ncauses,
...
arg.obs <- arg.csk <- arg.obs.no.censor <- 
  arg.gk.lm <- arg.gk.rf <-  
  arg.dw <-  arg.zom <- arg_list
...
#                paste(paste0(c(if(x>1) "lB_", paste0("Z", 1:ncov, "_")), x), collapse = " + ")) %>% 
#           as.formula
#       })
