...
# default settings
default <- list(n.eval = 300, n.sim = 1, tau = 10, n.stages = 2)
...
betas <- list(
  beta1 =
    list(beta.hazard0 = c(0, -1, -1 * c(1,1,1, 1, 1)),              # (int), surv.prev, covariate (1~5)
         ...
         beta.censor1 = c(-3, 0.2, 0.2 * c(1,1,1,-1,-1)))
    # beta1 =
...
p.list <- lapply(betas, function(x) length(x$beta.hazard0) - 2)
    ...
    setting = c(arg = list(arg), default, betas[[arg1]], p = p.list[[arg1]],
            propensity[[arg2]], size[[arg3]], crit[[1]])
    # setting = c(arg = list(arg), default, betas[[arg1]], p = p.list[[arg1]],
