...
for (cv in 1:K){
  ...
  test_ids <- sub_ids[cv]
  train_ids <- sub_ids[-cv]
  test_indices = cv # test_indices = folds[[cv]] # outsample person index
  ...
  print(paste("Train size:", length(unique(test$id)), "Unique IDs with", nrow(test), "records"))
