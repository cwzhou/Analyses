...
library(ggplot2); library(cowplot)
  ...
  mthd = c("CZMK", "CSK", "PMCR", "ZOM", "CSKZOM", "observed") #"GKRF", "GKLM", "DW",
  ...
  for (crit in c("mean", "surv")) {
    nm.tmp =
      sprintf("output/%s/values_%s_tau_2700%s_%s.rds", 
    ...
    data.long = 
      ...
      readRDS() %>% 
      as.data.frame %>% 
      dplyr::select(-cens1, -ns.CZMK, -ns.CSK, -ns.PMCR,) %>% # -ns.GK, -ns.GKLM
      mutate(rep = 1:n()) %>% 
